...
gap = -1                        #Want to add a feature that has the different PAM and BLOSUM paramters stored and use that as the substitution matrix instead
...
def SmithWaterman(seq1, seq2):
    ...
    for row in range(1, s1 + 1):
        for column in range(1, s2 + 1):
            if maximum < scoreMatrix[row][column]:
                maximum = scoreMatrix[row][column]           #max value is stored
    ...
    while scoreMatrix[i][j] != 0:
        ...
        #This is for if the sequences dont match
            ...
            tempList = [scoreMatrix[i - 1][j - 1], scoreMatrix[i - 1][j], scoreMatrix[i][j - 1]]
...
output1, output2, matchString, alignScore = SmithWaterman(seq1, seq2)
print(output1 + '\n' + output2)
